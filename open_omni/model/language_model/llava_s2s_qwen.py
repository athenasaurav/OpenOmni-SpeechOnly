...
def transition_attention_mask_pt(b, h, q_idx, kv_idx, prefix_length, channel, device):
    ...
    block_mask = channel_tensor[q_idx].unsqueeze(-1) == channel_tensor[kv_idx].unsqueeze(0)
    # Causal mask to prevent attending to future positions
    causal_mask = q_idx.unsqueeze(-1) >= kv_idx.unsqueeze(0)
    
    # Combine masks: prefix or block should be true, and must satisfy causal constraint
...
class LlavaS2SQwenConfig(Qwen2Config):
    ...
    def __init__(self, **kwargs):
        ...
        self.speech_encoder = getattr(kwargs, 'speech_encoder', None)
        self.speech_encoder_type = getattr(kwargs, 'speech_encoder_type', 'whisper')
        
        # Speech projector configuration
        self.speech_projector_type = getattr(kwargs, 'speech_projector_type', 'linear')
        
        # Speech generator configuration
        self.speech_generator_type = getattr(kwargs, 'speech_generator_type', 'ctc')
        
        # Speech-specific parameters
        self.speech_hidden_size = getattr(kwargs, 'speech_hidden_size', 1024)
        self.speech_select_layer = getattr(kwargs, 'speech_select_layer', -1)
        self.speech_select_feature = getattr(kwargs, 'speech_select_feature', 'hidden_states')
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if speeches is None:
                ...
                self.prepare_inputs_labels_for_speech_only(input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities, image_sizes=image_sizes, speeches=speeches, speech_lengths=speech_lengths)
            else:
                (input_ids, position_ids, attention_mask, past_key_values, inputs_embeds, labels) = \
                self.prepare_inputs_labels_for_speech_only(input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities, image_sizes, speeches, speech_lengths)
        ...
        if self.training:
            ...
            if tgt_units is not None:
                ctc_loss = self.model.speech_generator(output["hidden_states"][-1][..., :-1, :].contiguous(), labels[..., 1:].contiguous(), tgt_units)
        ...
        **kwargs,
        ...
        if images is not None or speeches is not None:
            if speeches is None:
                ...
                self.prepare_inputs_labels_for_speech_only(inputs, position_ids, attention_mask, None, None, images, modalities, image_sizes=image_sizes, speeches=speeches, speech_lengths=speech_lengths)
            else:
                (inputs, position_ids, attention_mask, _, inputs_embeds, _) = \
                self.prepare_inputs_labels_for_speech_only(inputs, position_ids, attention_mask, None, None, images, modalities, image_sizes=image_sizes, speeches=speeches, speech_lengths=speech_lengths)
        else:
