...
#SBATCH --gres=gpu:4
#SBATCH --output=./slurm_logs/finetune-openomni-visual.out
#SBATCH --error=./slurm_logs/finetune-openomni-visual.error.out
...
LLM_VERSION="checkpoints/Llama-3.1-8B-S2S-Omni" # make sure S2S in model name, we change the structure
...
MID_RUN_NAME="OpenOmni-8B-Llama3-Omni"
